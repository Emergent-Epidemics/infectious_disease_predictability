#SV Scarpino
#F2 supp big sweep for https://www.nature.com/articles/s41467-019-08616-0
...
if(run_new == TRUE){
  ...
  for(iter in 1:n_iter){
    for(i in 1:length(data)){
      ...
      ent.i <- rel_ent(x = x.i, m.min = 2, m.max = 7, t.min = 1, t.max = 12, do_mc_ent = FALSE)
...
if(run_big_model == TRUE){
  use_model <- which(RESULTS$disease %in% c("Chlamydia", "Gonorrhea", "Hepatitis A", "Influenza", "Measles", "Mumps", "Polio", "Whooping Cough", "Dengue"))
  
  data_model <- RESULTS[use_model, ]
...
Tukey_Model2 <- TukeyHSD(model2)
Tukey_Model2$`data_model2$disease`[which(Tukey_Model2$`data_model2$disease`[,"p adj"] < 0.05),]
...
p1 + geom_smooth(method = 'loess') + scale_colour_manual(values = pal, guide_legend(title = "Disease")) + xlab("Number of weeks") + ylab("Predictability (1 - H)") + theme(legend.position = c(0.86, 0.73), legend.key = element_rect(fill = "#f0f0f0"), legend.background = element_rect(fill = "#ffffffaa", colour = "black"), panel.background = element_rect(fill = "white", colour = "black"), axis.text.y = element_text(colour = "black", size = 14), axis.text.x = element_text(colour = "black", size = 14), axis.title = element_text(colour = "black", size = 20), panel.grid.minor = element_line(colour = "#00000050",linetype = 3), panel.grid.major = element_line(colour = "#00000060", linetype = 3)) + scale_y_continuous(expand = c(0.01,0.01)) + scale_x_continuous(expand = c(0.01,0.01))